"""
OCRBench v2 benchmark for Anthropic Claude.
10,000 QA pairs across 30 task types (EN + CN).

Usage:
    uv run -m benchmarks.ocrbench_v2.ocrbench_v2_anthropic
    uv run -m benchmarks.ocrbench_v2.ocrbench_v2_anthropic --predict-only
    uv run -m benchmarks.ocrbench_v2.ocrbench_v2_anthropic --evaluate-only
"""
...
PRED_OUTPUT = RESULTS_DIR / "ocrbench_v2_anthropic_predictions.json"
EVAL_OUTPUT = RESULTS_DIR / "ocrbench_v2_anthropic_scored.json"

sys.path.insert(0, str(PROJECT_ROOT))
from src.commons_anthropic import invoke_anthropic  # noqa: E402
...
def build_messages(question: str, image_b64: str) -> list[dict]:
    return [
        {
            "role": "user",
            "content": [
...
async def process_sample(sample_meta: dict, rate_limiter):
    messages = build_messages(sample_meta["question"], sample_meta["image_b64"])

    for attempt in range(MAX_RETRIES):
        await rate_limiter.acquire()
        try:
            response = await asyncio.to_thread(invoke_anthropic, messages, MODEL)
    ...
    for batch_start in tqdm(range(0, total, BATCH_SIZE), desc="Batches"):
        ...
        for i in range(len(batch["id"])):
            ...
            if sample_id in existing:
                continue
            image_b64 = pil_to_b64(batch["image"][i])
...
def run_evaluation():
    ...
    metrics_path = RESULTS_DIR / "ocrbench_v2_anthropic_metrics.json"
...
def main():
    parser = argparse.ArgumentParser(description="OCRBench v2 benchmark for Anthropic Claude")
    parser.add_argument(
