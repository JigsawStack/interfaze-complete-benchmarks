...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process a JSON file to calculate scores."
    )
    parser.add_argument(
        "--json_file",
        type=str,
    ...
    for item in data_list:
        if "ignore" in item.keys():
            ...
            item["type"] == "text recognition en"
            or item["type"] == "fine-grained text recognition en"
            or item["type"] == "full-page OCR en"
        ):
            en_text_recognition_list.append(item["score"])

        elif (
            item["type"] == "text grounding en"
            or item["type"] == "VQA with position en"
        ):
            ...
            item["type"] == "key information extraction en"
            or item["type"] == "key information mapping en"
        ):
            en_relationship_extraction_list.append(item["score"])

        elif (
            item["type"] == "document parsing en"
            or item["type"] == "chart parsing en"
            or item["type"] == "table parsing en"
            or item["type"] == "formula recognition en"
        ):
            ...
            item["type"] == "document classification en"
            or item["type"] == "cognition VQA en"
            or item["type"] == "diagram QA en"
        ):
            en_visual_text_understanding_list.append(item["score"])

        elif (
            item["type"] == "reasoning VQA en"
            or item["type"] == "science QA en"
            or item["type"] == "APP agent en"
            or item["type"] == "ASCII art classification en"
        ):
            ...
            item["type"] == "key information extraction cn"
            or item["type"] == "handwritten answer extraction cn"
        ):
            cn_relationship_extraction_list.append(item["score"])

        elif (
            item["type"] == "document parsing cn"
            or item["type"] == "table parsing cn"
            or item["type"] == "formula recognition cn"
        ):
            ...
            item["type"] == "reasoning VQA cn" or item["type"] == "text translation cn"
        ):
