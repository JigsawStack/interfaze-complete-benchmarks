...
def ocr_eval(references, predictions):
    ...
    for idx, ref_value in enumerate(references):
        ...
        for tmpidx, tmpref in enumerate(ref_values):
            tmppred = (
                pred_values[tmpidx] if tmpidx < len(pred_values) else pred_values[0]
            )
            if (
                len(pred_values) == 1 and tmppred != "None" and "None" not in ref_values
            ):  # pred 1, and not None
...
def csv_eval(predictions, references, easy, pred_type="json"):
    ...
    def get_ap(
        ...
        ap = len([num for num in sim_list if num >= sim_threhold]) / (
            len(sim_list) + 1e-16
        )
        return ap
