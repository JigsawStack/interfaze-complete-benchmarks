"""
OCRBench v2 benchmark for Interfaze.
10,000 QA pairs across 30 task types (EN + CN).

Usage:
    uv run python benchmarks/OCRBench_v2/ocrbench_v2.py
    uv run python benchmarks/OCRBench_v2/ocrbench_v2.py --predict-only
    uv run python benchmarks/OCRBench_v2/ocrbench_v2.py --evaluate-only
"""
...
PRED_OUTPUT = RESULTS_DIR / "ocrbench_v2_predictions.json"
EVAL_OUTPUT = RESULTS_DIR / "ocrbench_v2_scored.json"
...
async def run_predictions():
    ...
    # Resume: load existing predictions and only re-run failures
    existing = {}
    ...
    for batch_start in tqdm(range(0, total, BATCH_SIZE), desc="Batches"):
        ...
        # Encode images to base64 and extract metadata, skip already-done samples
        samples = []
        for i in range(len(batch["id"])):
            ...
            # Use custom prompt for text spotting EN
...
def run_evaluation():
    ...
    # spotting_metric.py uses relative paths, so chdir to benchmark dir
    import os
    original_cwd = os.getcwd()
    os.chdir(BENCHMARK_DIR)

    # Step 1: Score each sample using eval.py
    print("Step 1: Scoring individual samples...")
    from eval import process_predictions  # noqa: E402
    ...
    # Step 2: Compute overall metrics using get_score.py logic
    ...
    print(f"\n{'=' * 60}")
    print("OCRBench v2 Results (Interfaze)")
    print(f"{'=' * 60}")
    ...
    metrics_path = RESULTS_DIR / "ocrbench_v2_metrics.json"
...
def main():
    parser = argparse.ArgumentParser(description="OCRBench v2 benchmark for Interfaze")
    parser.add_argument(
