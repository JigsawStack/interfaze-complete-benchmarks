"""
OCRBench v2 benchmark for Kimi K2.6 via OpenRouter.
10,000 QA pairs across 30 task types (EN + CN).

Usage:
    uv run -m benchmarks.ocrbench_v2.ocrbench_v2_kimi
    uv run -m benchmarks.ocrbench_v2.ocrbench_v2_kimi --predict-only
    uv run -m benchmarks.ocrbench_v2.ocrbench_v2_kimi --evaluate-only
"""
...
PRED_OUTPUT = RESULTS_DIR / "ocrbench_v2_kimi_predictions.json"
EVAL_OUTPUT = RESULTS_DIR / "ocrbench_v2_kimi_scored.json"
...
def run_evaluation():
    ...
    metrics_path = RESULTS_DIR / "ocrbench_v2_kimi_metrics.json"
...
def main():
    parser = argparse.ArgumentParser(description="OCRBench v2 benchmark for Kimi K2.6 via OpenRouter")
    parser.add_argument(
