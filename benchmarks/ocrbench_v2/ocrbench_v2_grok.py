"""
OCRBench v2 benchmark for Grok 4.3 via OpenRouter.
10,000 QA pairs across 30 task types (EN + CN).

Usage:
    uv run -m benchmarks.ocrbench_v2.ocrbench_v2_grok
    uv run -m benchmarks.ocrbench_v2.ocrbench_v2_grok --predict-only
    uv run -m benchmarks.ocrbench_v2.ocrbench_v2_grok --evaluate-only
"""
...
PRED_OUTPUT = RESULTS_DIR / "ocrbench_v2_grok_predictions.json"
EVAL_OUTPUT = RESULTS_DIR / "ocrbench_v2_grok_scored.json"
...
def run_evaluation():
    ...
    metrics_path = RESULTS_DIR / "ocrbench_v2_grok_metrics.json"
...
def main():
    parser = argparse.ArgumentParser(description="OCRBench v2 benchmark for Grok 4.3 via OpenRouter")
    parser.add_argument(
