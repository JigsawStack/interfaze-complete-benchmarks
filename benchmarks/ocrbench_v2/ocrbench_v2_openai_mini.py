"""
OCRBench v2 benchmark for OpenAI gpt-5.4-mini.
10,000 QA pairs across 30 task types (EN + CN).

Usage:
    uv run -m benchmarks.ocrbench_v2.ocrbench_v2_openai_mini
    uv run -m benchmarks.ocrbench_v2.ocrbench_v2_openai_mini --predict-only
    uv run -m benchmarks.ocrbench_v2.ocrbench_v2_openai_mini --evaluate-only
"""
...
PRED_OUTPUT = RESULTS_DIR / "ocrbench_v2_openai_mini_predictions.json"
EVAL_OUTPUT = RESULTS_DIR / "ocrbench_v2_openai_mini_scored.json"
...
def run_evaluation():
    ...
    metrics_path = RESULTS_DIR / "ocrbench_v2_openai_mini_metrics.json"
...
def main():
    parser = argparse.ArgumentParser(description="OCRBench v2 benchmark for OpenAI gpt-5.4-mini")
    parser.add_argument(
