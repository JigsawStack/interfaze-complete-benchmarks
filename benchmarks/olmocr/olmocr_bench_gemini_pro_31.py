"""
OlmOCR Benchmark for Gemini 3.1 Pro Preview.

Mirrors olmocr_bench.py (interfaze) but uses gemini-3.1-pro-preview.

Usage:
    uv run -m benchmarks.olmocr.olmocr_bench_gemini_pro_31
    uv run -m benchmarks.olmocr.olmocr_bench_gemini_pro_31 --sample
    uv run -m benchmarks.olmocr.olmocr_bench_gemini_pro_31 --skip-generation
    uv run -m benchmarks.olmocr.olmocr_bench_gemini_pro_31 --generate-only
"""
...
async def process_page(pdf_path, page_num, output_path, rate_limiter):
    from olmocr.bench.runners.run_gemini_pro_31 import run_gemini_pro_31

    for attempt in range(MAX_RETRIES):
        await rate_limiter.acquire()
        try:
            result = await asyncio.to_thread(run_gemini_pro_31, pdf_path, page_num)
