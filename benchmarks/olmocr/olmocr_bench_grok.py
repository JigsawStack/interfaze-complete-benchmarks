"""
OlmOCR Benchmark for OpenAI Grok 4.3.

Mirrors olmocr_bench.py (interfaze) but uses Grok 4.3 via OpenAI.

Usage:
    uv run -m benchmarks.olmocr.olmocr_bench_grok
    uv run -m benchmarks.olmocr.olmocr_bench_grok --sample
    uv run -m benchmarks.olmocr.olmocr_bench_grok --skip-generation
    uv run -m benchmarks.olmocr.olmocr_bench_grok --generate-only
"""
...
async def process_page(pdf_path, page_num, output_path, rate_limiter):
    from olmocr.bench.runners.run_grok import run_grok

    for attempt in range(MAX_RETRIES):
        await rate_limiter.acquire()
        try:
            result = await asyncio.to_thread(run_grok, pdf_path, page_num)
