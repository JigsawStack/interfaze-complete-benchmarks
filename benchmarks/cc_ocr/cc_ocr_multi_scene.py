...
def evaluate_results(results: list[dict]) -> dict:
    ...
    # multi_scene_ocr: all word-level, lowercase, alphanum-only
    is_word_level = True
    is_lower = True
    ...
    for cat, samples in by_category.items():
        ...
        per_cat_metrics[cat] = calculate_metrics(pred_info, gt_info, is_verbose=True)

    overall_metrics = calculate_metrics(all_pred_tokens, all_gt_tokens, is_verbose=True)
    return {"overall": overall_metrics, "by_category": per_cat_metrics}


def print_summary(metrics: dict, num_samples: int, num_failures: int):
    print("CC-OCR Multi-Scene OCR Benchmark Results (Interfaze)")
    print(f"Samples: {num_samples} | Failures: {num_failures}")
    print(f"{'Category':<20} {'Macro F1':>10} {'Micro F1':>10} {'Precision':>10} {'Recall':>10}")
    for cat in sorted(metrics["by_category"]):
        m = metrics["by_category"][cat]
        print(f"{cat:<20} {m['macro_f1_score']:>10.4f} {m['micro_f1_score']:>10.4f} {m['micro_precision']:>10.4f} {m['micro_recall']:>10.4f}")
    o = metrics["overall"]
    print(f"{'OVERALL':<20} {o['macro_f1_score']:>10.4f} {o['micro_f1_score']:>10.4f} {o['micro_precision']:>10.4f} {o['micro_recall']:>10.4f}")


async def run_benchmark():
    print("Loading CC-OCR dataset (multi_scene_ocr)...")
    dataset = load_dataset("wulipc/CC-OCR", "multi_scene_ocr")
    ...
    with open(RESULTS_DIR / "cc_ocr_multi_scene_responses.json", "w") as f:
    ...
    with open(RESULTS_DIR / "cc_ocr_multi_scene_metrics.json", "w") as f:
