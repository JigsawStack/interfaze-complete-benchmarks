"""
RefCOCO benchmark for OpenAI GPT-5 series.
...
Default model: `gpt-5.5`. Other models can be selected with `--model`, e.g.
`--model gpt-5.4-mini`. Reasoning defaults to fully off (`reasoning_effort="none"`),
which is accepted by the GPT-5.x family. Temperature pinned to 0.0.

Usage:
    uv run -m benchmarks.obj_detection.refcoco_openai --split testA
    uv run -m benchmarks.obj_detection.refcoco_openai --split testA --model gpt-5.4-mini
    uv run -m benchmarks.obj_detection.refcoco_openai --split testA --limit 20
    uv run -m benchmarks.obj_detection.refcoco_openai --split testA --evaluate-only

Env: OPENAI_API_KEY must be set (loaded from .env).
"""
...
# Mutated by CLI before any inference runs.
MODEL = DEFAULT_MODEL
REASONING_EFFORT = DEFAULT_REASONING_EFFORT


def model_slug(model: str) -> str:
    """Compact model id used in output filenames. Matches existing convention:
...
# Override OPENAI_BASE_URL — `.env` points it at the interfaze worker for the
# main benchmark; we need to hit the real OpenAI API here.
...
def invoke_openai(messages: list[dict]):
    """Single chat.completions call against the selected GPT-5.x model.

    GPT-5.x rejects non-default temperature when reasoning is engaged. We pass
    `temperature=0.0` only when reasoning is fully off ('none'); otherwise we
    omit it and let the API default apply."""
    kwargs = {
        "model": MODEL,
        "messages": messages,
...
def print_summary(metrics: dict, dataset_name: str, split: str):
    print(f"\n{'=' * 60}")
    print(f"Grounding Results — {dataset_name} / {split} ({MODEL}, reasoning={REASONING_EFFORT})")
    print(f"{'=' * 60}")
...
def build_tag(dataset: str, split: str) -> str:
    ...
    return f"{model_slug(MODEL)}_{ds_slug}_{split}"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="RefCOCO benchmark for OpenAI GPT-5.x "
