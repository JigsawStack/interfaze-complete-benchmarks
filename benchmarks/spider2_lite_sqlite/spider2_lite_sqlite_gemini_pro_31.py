"""
Spider 2.0-Lite (SQLite-only) benchmark for Gemini 3.1 Pro Preview.

Mirrors spider2_lite_sqlite.py but uses gemini-2.5-pro via google-genai.

Usage:
    uv run -m benchmarks.spider2_lite_sqlite.spider2_lite_sqlite_gemini_pro_31
    uv run -m benchmarks.spider2_lite_sqlite.spider2_lite_sqlite_gemini_pro_31 --predict-only
    uv run -m benchmarks.spider2_lite_sqlite.spider2_lite_sqlite_gemini_pro_31 --evaluate-only
    uv run -m benchmarks.spider2_lite_sqlite.spider2_lite_sqlite_gemini_pro_31 --limit 5
"""
...
PRED_SQL_DIR = RESULTS_DIR / "spider2_lite_sqlite_gemini_pro_31_predictions"
PRED_META_PATH = RESULTS_DIR / "spider2_lite_sqlite_gemini_pro_31_predictions.json"
EXEC_CSV_DIR = RESULTS_DIR / "spider2_lite_sqlite_gemini_pro_31_exec"
EVAL_OUTPUT = RESULTS_DIR / "spider2_lite_sqlite_gemini_pro_31_scored.json"
METRICS_OUTPUT = RESULTS_DIR / "spider2_lite_sqlite_gemini_pro_31_metrics.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Spider 2.0-Lite SQLite-only benchmark for Gemini 3.1 Pro Preview"
    )
